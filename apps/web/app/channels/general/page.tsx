...
export default function GeneralChannelsPage() {
  ...
  const filtered = useMemo(() => {
    const list = channels
      .filter((c) => (c.category ?? 'general') === 'general')
      .filter((c) => c.name.toLowerCase().includes(query.toLowerCase()));

    // Keep core General first, then support, maintenance, then others A-Z
    ...
    return list.sort((a, b) => {
      ...
      const ap = priority.get(an);
      const bp = priority.get(bn);
        ...
        <div className="mb-4 flex items-center gap-2">
          <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-gray-800 text-sm text-gray-300 w-full max-w-md">
            ...
            <input
              ...
              placeholder="Search General channels"
